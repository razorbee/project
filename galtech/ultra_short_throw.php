
<?php include('head.php');?>
<?php include('header.php');?>


	<div class="bg"><img src="img/4.jpg" class="img-fluid" style="filter: blur(1px);
  -webkit-filter: blur(1px);">
       <div class="hero-text">
    <h1 style="color:black">Ultra Short Throw</h1>
    
  </div>

    </div>

<div class="container">
    <div class="row">
		
        div class="btn-group btn-breadcrumb breadcrumb-default" style="margin-left:-220px;">
            <a href="index.php" class="btn btn-default"><i class="glyphicon glyphicon-home"></i></a>
            <a href="#" class="btn btn-default visible-lg-block visible-md-block"><b>Products</b></a>
            <a href="#" class="btn btn-default visible-lg-block visible-md-block"><b>Multimedia Projector</b></a>
           <div class="btn btn-default visible-xs-block hidden-xs visible-sm-block ">...</div>
            <div class="btn btn-info1"><b>Ultra Short Throw</b></div>
        </div>
	</div>

<section class="blog-section">
<div class="container">

<div class="row">
   
   <!--1 BEGINS-->     
        
<div class="col-md-4 col-sm-6 col-xs-12">
<div class="default-blog-news">
<figure class="img-holder">
<img src="img/products/cp_tw3005ef.jpg" alt="products" class="img-responsive">
</figure>
<div class="lower-content">
<h3>CP-AX2505EF</h3>
<h4>Ultra Short Throw</h4>
<div class="text">
    <div class="text-box">
<p>2,700 ANSI Lumens, XGA</p>

<p>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> 2,700 ANSI Lumens, XGA<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> 10,000:1 High Contrast Ratio<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> Accentualizer<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> DICOM Simulation Mode<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> Image Optimizer<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> Perfect Fit<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> HDMI Input<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> Intelligent Eco and Saver Mode<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> High Reliability and Stability</p>
    </div>
<a href="img/products/cp_ax2505ef.pdf" target="_blank" class="btn btn-danger know">Know More</a>
<a href="contact.php" class="btn btn-info">Buy Now</a>
</div>
</div>
</div>
</div>
    
    
<!--1 ENDS-->

        
<!--2 BEGINS-->             
<div class="col-md-4 col-sm-6 col-xs-12">
<div class="default-blog-news">
<figure class="img-holder">
<img src="img/products/cp_tw3005ef.jpg" alt="products" class="img-responsive">
</figure>
<div class="lower-content">
<h3>CP-AX3005EF</h3>
<h4>Ultra Short Throw</h4>
<div class="text">
    <div class="text-box">
<p>3,300 ANSI Lumens, XGA</p>
<p>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> 3,300 ANSI Lumens, XGA<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> 10,000:1 High Contrast Ratio<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> Accentualizer<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> DICOM Simulation Mode<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> Image Optimizer<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> Perfect Fit<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> HDMI Input<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> Intelligent Eco and Saver Mode<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> High Reliability and Stability</p>
    </div>
<a href="img/products/cp_ax2505ef.pdf" target="_blank" class="btn btn-danger know">Know More</a>
<a href="contact.php" class="btn btn-info">Buy Now</a>
</div>
</div>
</div>
</div>
<!--2 ENDS-->


 
 
<!--3 BEGINS-->           
<div class="col-md-4 col-sm-6 col-xs-12">
<div class="default-blog-news">
<figure class="img-holder">
<img src="img/products/cp_tw3005ef.jpg" alt="products" class="img-responsive">
</figure>
<div class="lower-content">
<h3>CP-AX3505EF</h3>
<h4>Ultra Short Throw</h4>
<div class="text">
    <div class="text-box">
<p>3,600 ANSI Lumens, XGA</p>
<p>

<i style="color:#68a4e2;" class="fa fa-caret-right"></i> 3,600 ANSI Lumens, XGA<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> 10,000:1 High Contrast Ratio<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> Accentualizer<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> DICOM Simulation Mode<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> Image Optimizer<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> Perfect Fit<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> HDMI Input<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> Intelligent Eco and Saver Mode<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> High Reliability and Stability</p>
    </div>
<a href="img/products/cp_ax2505ef.pdf" target="_blank" class="btn btn-danger know">Know More</a>
<a href="contact.php" class="btn btn-info">Buy Now</a>
</div>
</div>
</div>
</div>
<!--3 ENDS-->



<div class="clearfix"></div>


            
            
            
<!--4 BEGINS-->           
<div class="col-md-4 col-sm-6 col-xs-12">
<div class="default-blog-news">
<figure class="img-holder">
<img src="img/products/cp_tw3005ef.jpg" alt="products" class="img-responsive">
</figure>
<div class="lower-content">
<h3>CP-AW2505EF</h3>
<h4>Ultra Short Throw</h4>
<div class="text">
    <div class="text-box">
<p>2,700 ANSI Lumens, WXGA</p>

<p>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> 2,700 ANSI Lumens, WXGA<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> 10,000:1 High Contrast Ratio<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> Accentualizer<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> DICOM Simulation Mode<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> Image Optimizer<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> Perfect Fit<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> HDMI Input<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> Intelligent Eco and Saver Mode<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> High Reliability and Stability</p>
    </div>
<a href="img/products/cp_ax2505ef.pdf" target="_blank" class="btn btn-danger know">Know More</a>
<a href="contact.php" class="btn btn-info">Buy Now</a>
</div>
</div>
</div>
</div>
<!--4 ENDS-->  

            
<!--5 BEGINS-->           
<div class="col-md-4 col-sm-6 col-xs-12">
<div class="default-blog-news">
<figure class="img-holder">
<img src="img/products/cp_tw3005ef.jpg" alt="products" class="img-responsive">
</figure>
<div class="lower-content">
<h3>CP-AW3005EF</h3>
<h4>Ultra Short Throw</h4>
<div class="text">
    <div class="text-box">
<p>3,300 ANSI Lumens, WXGA</p>
<p>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> 10,000:1 High Contrast Ratio<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> Accentualizer<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> DICOM Simulation Mode<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> Image Optimizer<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> Perfect Fit<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> HDMI Input<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> Intelligent Eco and Saver Mode<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> High Reliability and Stability</p>
    </div>
<a href="img/products/cp_ax2505ef.pdf" target="_blank" class="btn btn-danger know">Know More</a>
<a href="contact.php" class="btn btn-info">Buy Now</a>
</div>
</div>
</div>
</div>
<!--5 ENDS--> 

	 </div>
    </div>
</section>




<?php include('footer.php');?>
<?php include('script.php');?>