<?php include('head.php');?>
<?php include('header.php');?>

<div class="bg"><img src="img/4.jpg" class="img-fluid" style="filter: blur(1px);
  -webkit-filter: blur(1px);">
       <div class="hero-text">
    <h1 style="color:black">Super Short Throw</h1>
    <div class="btn-group btn-breadcrumb breadcrumb-default" style="margin-left:-220px;">
            <a href="index.php" class="btn btn-default"><i class="glyphicon glyphicon-home"></i></a>
            <a href="#" class="btn btn-default visible-lg-block visible-md-block"><b>Products</b></a>
            <a href="#" class="btn btn-default visible-lg-block visible-md-block"><b>Multimedia Projector</b></a>
           <div class="btn btn-default visible-xs-block hidden-xs visible-sm-block ">...</div>
            <div class="btn btn-info1"><b>Super Short Throw</b></div>
        </div>
  </div>

    </div>

<section class="blog-section">
<div class="container">

<div class="row">
   
   <!--1 BEGINS-->     
        
<div class="col-md-4 col-sm-6 col-xs-12">
<div class="default-blog-news">
<figure class="img-holder">
<img src="img/products/cp-cx250_cx300_cw250_cw300-800.jpg" alt="products" class="img-responsive">
</figure>
<div class="lower-content">
<h3>CP-CX251EF</h3>
<h4>Super Short Throw</h4>
<div class="text">
<p>Super Short Throw, 2,500 ANSI Lumens, XGA</p>

<p>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> 2,500 ANSI Lumens, XGA<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> Super Short Throw Length<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> Hybrid Air Filter<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> Easy Maintenance<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> Saver Mode<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> Intelligent Eco Mode</p>
<a href="img/products/Brochure1_toshiba.pdf" target="_blank" class="btn btn-danger know  know">Know More</a>
<a href="contact.php" class="btn btn-info">Buy Now</a>
</div>
</div>
</div>
</div>
    
    
<!--1 ENDS-->

        
<!--2 BEGINS-->             
<div class="col-md-4 col-sm-6 col-xs-12" >
<div class="default-blog-news">
<figure class="img-holder">
<img src="img/products/cp-cx250_cx300_cw250_cw300-800.jpg" alt="products" class="img-responsive">
</figure>
<div class="lower-content">
<h3>CP-CX301WNEF</h3>
<h4>Super Short Throw</h4>
<div class="text">
<p>Super Short Throw, 3,100 ANSI Lumens, XGA</p>
<p>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> 3,100 ANSI Lumens, XGA<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> Super Short Throw Length<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> Hybrid Air Filter<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> Easy Maintenance<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> Saver Mode<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> Intelligent Eco Mode</p>
<a href="img/products/Brochure1_toshiba.pdf" target="_blank" class="btn btn-danger know  know">Know More</a>
<a href="contact.php" class="btn btn-info">Buy Now</a>
</div>
</div>
</div>
</div>
<!--2 ENDS-->


 
 
<!--3 BEGINS-->           
<div class="col-md-4 col-sm-6 col-xs-12">
<div class="default-blog-news">
<figure class="img-holder">
<img src="img/products/cp-cx250_cx300_cw250_cw300-800.jpg" alt="products" class="img-responsive">
</figure>
<div class="lower-content">
<h3>CP-CW251WN</h3>
<h4>Super Short Throw</h4>
<div class="text">
<p>Super Short Throw, 2,600 ANSI Lumens, WXGA</p>
<p>

<i style="color:#68a4e2;" class="fa fa-caret-right"></i> Super Short Throw Length<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> Hybrid Air Filter<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> Easy Maintenance<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> Saver Mode<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> Intelligent Eco Mode</p>
    <br>
<a href="img/products/Brochure1_toshiba.pdf" target="_blank" class="btn btn-danger know">Know More</a>
<a href="contact.php" class="btn btn-info">Buy Now</a>
</div>
</div>
</div>
</div>
<!--3 ENDS-->



<div class="clearfix"></div>


            
            
            
<!--4 BEGINS-->           
<div class="col-md-4 col-sm-6 col-xs-12" >
<div class="default-blog-news">
<figure class="img-holder">
<img src="img/products/cp-cx250_cx300_cw250_cw300-800.jpg" alt="products" class="img-responsive">
</figure>
<div class="lower-content">
<h3>CP-BX301WNEF</h3>
<h4>Super Short Throw</h4>
<div class="text">
<p>Super Short Throw, 3,200 ANSI Lumens,<br>
 XGA, Wireless Ready</p>

<p>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> 3,200 ANSI Lumens, XGA<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> Short Throw Length<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> 10,000:1 High Contrast Ratio<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> Installability and System Features<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> Easy Maintenance<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> Various Network Functions<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> Saver Mode<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> Intelligent Eco Mode<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> Hybrid Air Filter<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> Wireless Capability</p>
<a href="img/products/Brochure1_toshiba.pdf" target="_blank" class="btn btn-danger  know">Know More</a>
<a href="contact.php" class="btn btn-info">Buy Now</a>
</div>
</div>
</div>
</div>
<!--4 ENDS-->  

            
<!--5 BEGINS-->           
<div class="col-md-4 col-sm-6 col-xs-12">
<div class="default-blog-news">
<figure class="img-holder">
<img src="img/products/cp-cx250_cx300_cw250_cw300-800.jpg" alt="products" class="img-responsive">
</figure>
<div class="lower-content">
<h3>CP-BW301WNEF</h3>
<h4>Super Short Throw</h4>
<div class="text">
<p>Super Short Throw, 3,000 ANSI Lumens, WXGA</p>
<p>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> 3,000 ANSI Lumens, XGA<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> Short Throw Length<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> 10,000:1 High Contrast Ratio<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> Installability and System Features<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> Easy Maintenance<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> Various Network Functions<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> Saver Mode<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> Intelligent Eco Mode<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> Hybrid Air Filter<br>
<i style="color:#68a4e2;" class="fa fa-caret-right"></i> Wireless Capability</p>
<a href="img/products/Brochure1_toshiba.pdf" target="_blank" class="btn btn-danger  know">Know More</a>
<a href="contact.php" class="btn btn-info">Buy Now</a>
</div>
</div>
</div>
</div>
<!--5 ENDS--> 

	 </div>
    </div>
</section>
<?php include('footer.php');?>
<?php include('script.php');?>