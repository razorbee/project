...
class DashboardEssential
{
    ...
    public function envato_theme_license_dashboard_update_message_cb($plugin_data, $result)
    {
        ...
        $status = get_option('envato_theme_license_key_status');
        if ($status != 'valid') {
            echo sprintf(__('To receive automatic updates license activation is required. Please visit <a href="%sadmin.php?page=%sproduct-registration">Setting</a> page.', 'electrician'), admin_url(), $this->menu_slug);
        ...
        if (isset($_GET['settings-updated'])) {
            ...
            <?php } else {?>
                    <div class="notice notice-error">
                        <p><strong><?php esc_html_e($status, 'electrician')?><strong></p>
                    </div>
            <?php }
            } else {
    ...
    public function envato_theme_license_dashboard_theme_activate_license()
    {
        if (isset($_POST['envato_theme_theme_license_activate'])) {
            ...
            if(isset($_POST['envato_theme_theme_license_activate_checkbox']) && $_POST['envato_theme_theme_license_activate_checkbox']== 1){
   ...
   public function envato_theme_license_dashboard_sample_admin_notice()
   {
       ...
       if ($status != 'valid' || $purchase_key == '' || $this->token == '') {
           ...
           <p><strong><span class="setting-error-notice-heading" style="margin-top:-0.4em"><?php echo esc_html__('Require Activation','electrician');?></span><span style="display: block; margin: 0.5em 0.5em 0 0; clear: both;"><?php echo sprintf(__( "%s Theme Need to active with purchase code. Otherwise you can't Active / Update Bundle Plugin. You can active from <a href='".esc_url( admin_url().'admin.php?page=%sproduct-registration')."'>Here</a>.", "electrician"),$this->dashboard_Name,$this->menu_slug) ?> </span>
           </strong></p><button type="button" class="notice-dismiss"><span class="screen-reader-text"><?php echo esc_html__('Dismiss this notice.', 'electrician') ?> </span></button></div>
               <?php
       }
   }
    ...
    public function envato_theme_license_dashboard_style()
    {
        ...
        wp_localize_script($this->menu_slug_dashboard . '-js', 'ajax_dashboard_js', array('copytext' => esc_html__('Copied!', 'electrician')));
    }
    ...
    public function envato_theme_license_dashboard_get_tabs($activetab)
    {
        ...
        $tabarray = array('start' => array('title' => esc_html__('Getting Started', 'electrician'), 'link' => '?page=' . $this->menu_slug_dashboard),
            'registration' => array('title' => esc_html__('Registration', 'electrician'), 'link' => '?page=' . $this->menu_slug . 'product-registration'),
            'systemstatus' => array('title' => esc_html__('System Status', 'electrician'), 'link' => '?page=' . $this->menu_slug . 'system-status'),
            'plugin' => array('title' => esc_html__('Plugins', 'electrician'), 'link' => '?page=' . $this->menu_slug . 'install-required-plugins'));

        if (class_exists('OCDI_Plugin') && $this->token != '') {
            $tabarray['demo'] = array('title' => esc_html__('Demo Import', 'electrician'), 'link' => '?page=' . $this->menu_slug . 'one-click-demo-import');
        } else {
            $tabarray['demo'] = array('title' => esc_html__('Demo Import', 'electrician'), 'link' => '?page=' . $this->menu_slug . 'demo-content-install');
        }
        if (class_exists('iconmoonFontAdd') && $this->token != '') {
            $tabarray['icon'] = array('title' => esc_html__('Icon Add', 'electrician'), 'link' => '?page=custom-icon-upload');
        }
        $tabarray['support'] = array('title' => esc_html__('Support', 'electrician'), 'link' => '?page=' . $this->menu_slug . 'support');
        ?>
