...
if( isset( $portfolio_details ) && $portfolio_details == 'yes' ) {
	...
	if( isset( $portfolio_btn_text ) && $portfolio_btn_text == '' ) {
		...
		$portfolio_btn_text = esc_html__( 'View Project', 'wedding' );
	...
	if( isset( $related_slider ) && $related_slider == '' ) {
		...
		$related_slider_opt = wedding_get_theme_option( 'zozo_portfolio_related_slider' );
	...
	$cat_label 				= wedding_get_theme_option( 'zozo_portfolio_category_label' );



	$tag_label 				= wedding_get_theme_option( 'zozo_portfolio_tag_label' );



	$client_label			= wedding_get_theme_option( 'zozo_portfolio_client_label' );



	$date_label				= wedding_get_theme_option( 'zozo_portfolio_date_label' );



	$est_label				= wedding_get_theme_option( 'zozo_portfolio_estimation_label' );



	$dur_label				= wedding_get_theme_option( 'zozo_portfolio_duration_label' );
						...
						<?php if ( have_posts() ):
							...
							if( isset( $media_type ) && $media_type != 'none' ) {
								...
								if( isset( $media_type ) && $media_type == 'carousel' ) {
									...
									$media_content .= wedding_portfolio_media_output( $post->ID, 'single', '', $post->ID );
								...
								// Featured Image
									...
									$portfolio_full_img = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'wedding-portfolio-single' );
								...
								// Stack Images
									...
									foreach( $media_images as $image ) {
										...
										$post_image = wedding_get_portfolio_post_media( '', '', $image, 'wedding-portfolio-single', '1300', '500');
									...
									$media_content .= '<div class="zozo-isotope-wrapper portfolio-isotope-wrapper">';
										...
										foreach( $media_images as $image ) {
											...
											if( isset( $isotope_type ) && $isotope_type == 'masonry' ) {
												...
												$post_image = wedding_get_portfolio_post_media( '500', '', $image );
												...
												$post_image = wedding_get_portfolio_post_media( '', '', $image, 'wedding-portfolio-mid', '560', '385');
							...
							if( isset( $portfolio_details ) && $portfolio_details == 'yes' ) {
								...
								if( isset( $portfolio_custom_text ) && $portfolio_custom_text != '' ) {
									...
									$details_content .= '<p class="portfolio-custom-text">'. force_balance_tags( $portfolio_custom_text ) .'</p>';
								...
								if( isset( $portfolio_share ) && $portfolio_share == 1 ) {
									...
									$details_content .= wedding_display_social_sharing_icons( 'no' );
							...
							if( isset( $related_slider ) && $related_slider != 1 ) {
								...
								wedding_related_portfolio_slider();
							...
							if( wedding_get_theme_option( 'zozo_portfolio_prev_next' ) == 1 ) {



								wedding_postnavigation();
