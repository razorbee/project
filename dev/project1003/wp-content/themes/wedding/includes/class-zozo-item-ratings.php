...
class WeddingItemRatings {
	...
	protected $plugin_path						= WEDDING_THEME_DIR;



	protected $plugin_url						= WEDDING_THEME_URL;
	...
	*/
	...
	public function enqueue_admin_scripts() {
		...
		$js_inc_path 	= WEDDING_THEME_URL . '/js/';



		$css_inc_path 	= WEDDING_THEME_URL . '/css/';
		...
		wp_enqueue_script( 'wedding-zozo-rate-it', $js_inc_path . 'rate-it/wedding-jquery.rateit.min.js', array('jquery'), '1.0.22' );
		...
		wp_enqueue_script( 'wedding-zozo-item-ratings', $js_inc_path . 'rate-it/wedding-zozo-ratings-script.js', array('wedding-zozo-rate-it'), '1.0' );
		...
		wp_enqueue_style( 'wedding-zozo-item-ratings-stars', $css_inc_path . 'wedding-rateit-admin.css', array(), '1.0' );
	...
	*/
	...
	private function localize_script() {
		...
		wp_localize_script( 'wedding-zozo-item-ratings', $object, $js_vars );
	...
	*/
	...
	public function add_custom_column_content( $config_option_key, $column_name, $post_ID ) {
		...
		if ($column_name == $column_slug) {  
	        ...
	        echo apply_filters( 'wedding_item_ratings_column_stars', $output, $column_name, $option, $post_ID );
	...
	public function add_custom_meta_box( $config_option_key ) {
		...
		foreach( self::$class_config as $option ) {
			...
			if( $option['meta_key'] === $config_option_key ) {
		        ...
		        echo apply_filters( 'wedding_item_ratings_metabox_stars', $output, $option, $post_ID );
