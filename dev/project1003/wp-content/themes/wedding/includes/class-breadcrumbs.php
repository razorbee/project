...
if ( ! class_exists( 'Wedding_Breadcrumbs' ) ) {



	class Wedding_Breadcrumbs {
		...
		public function __construct() {
			...
			$defaults = array(
				 ...
				 'home_label'      			=> esc_html__( 'Home', 'wedding' ),



				 'tag_archive_prefix'   	=> esc_html__( 'Tag:', 'wedding' ),



				 'search_prefix'        	=> esc_html__( 'Search:', 'wedding' ),



				 'error_prefix' 			=> esc_html__( '404 - Page not Found', 'wedding' ),
			...
			$args 		= apply_filters( 'wedding_breadcrumbs_defaults', $defaults );
		...
		public function wedding_display_breadcrumbs() {
			...
			if ( function_exists('yoast_breadcrumb') && $options && $options['breadcrumbs-enable'] === true ) {
				...
				$this->wedding_display_theme_breadcrumbs();
		...
		private function wedding_display_theme_breadcrumbs() {
			...
			$this->breadcrumb_markup .= $this->wedding_get_breadcrumb_home();
			...
			if ( class_exists( 'bbPress' ) && is_bbpress() && ( bbp_is_topic_archive() || bbp_is_single_user() || bbp_is_search() ) ) {
				...
				$this->breadcrumb_markup .= $this->wedding_get_bbpress_main_archive_page();
			...
			if ( is_singular() ) {
				...
				if ( isset( $this->post->post_type ) && get_post_type_archive_link( $this->post->post_type ) &&	$this->show_post_type_archive ) {
					...
					$this->breadcrumb_markup .= $this->wedding_get_post_type_archive();
				...
				if ( isset( $this->post->post_parent ) && $this->post->post_parent == 0 ) {
					...
					$this->breadcrumb_markup .= $this->wedding_get_post_terms();
				...
				// If there are parents; mostly for pages
					...
					$this->breadcrumb_markup .= $this->wedding_get_post_ancestors();
				...
				$this->breadcrumb_markup .= $this->wedding_get_breadcrumb_leaf_markup();
				...
				if ( is_post_type_archive() ) {
					...
					$this->breadcrumb_markup .= $this->wedding_get_post_type_archive( FALSE );
				...
				// Taxonomy Archives
					...
					$this->breadcrumb_markup .= $this->wedding_get_taxonomies();



					$this->breadcrumb_markup .= $this->wedding_get_breadcrumb_leaf_markup( 'term' );
				...
				// Date Archives
					...
					if ( is_year() ) {
						...
						$this->breadcrumb_markup .= $this->wedding_get_breadcrumb_leaf_markup( 'year' );
					...
					// Month Archive, needs year link and month leaf
						...
						$this->breadcrumb_markup .= $this->wedding_get_single_breadcrumb_markup( $year, get_year_link( $year ) );



						$this->breadcrumb_markup .= $this->wedding_get_breadcrumb_leaf_markup( 'month' );
					...
					// Day Archive, needs year and month link and day leaf
						...
						$this->breadcrumb_markup .= $this->wedding_get_single_breadcrumb_markup( $month_name, get_month_link( $year, $month ) );



						$this->breadcrumb_markup .= $this->wedding_get_breadcrumb_leaf_markup( 'day' );						
				...
				// Author Archives
					...
					$this->breadcrumb_markup .= $this->wedding_get_breadcrumb_leaf_markup( 'author' );
				...
				// Search Page
					...
					$this->breadcrumb_markup .= $this->wedding_get_breadcrumb_leaf_markup( 'search' );
				...
				// 404 Page
					...
					if ( class_exists( 'TribeEvents' ) && ( tribe_is_event() || wedding_is_events_archive() ) ) { 



						$this->breadcrumb_markup .= $this->wedding_get_breadcrumb_leaf_markup( 'events' );
					...
					// Default case
						...
						$this->breadcrumb_markup .= $this->wedding_get_breadcrumb_leaf_markup( '404' );
				...
				// bbPress
				 	...
				 	if ( bbp_is_search() ) {
				 		...
				 		$this->breadcrumb_markup .= $this->wedding_get_breadcrumb_leaf_markup( 'bbpress_search' );
				 	...
				 	// User page
						...
						$this->breadcrumb_markup .= $this->wedding_get_breadcrumb_leaf_markup( 'bbpress_user' );
		...
		private function wedding_get_breadcrumb_home() {
			...
			if ( ! is_front_page() ) {
				...
				$home_link = $this->wedding_get_single_breadcrumb_markup( $this->home_label, get_home_url() );
			...
			// If the home page is the main blog page
				...
				$blog_title = wedding_get_theme_option( 'zozo_blog_title' );
				...
				if( isset( $blog_title ) && $blog_title == '' ) {
					...
					$blog_title = esc_html__( 'Home', 'wedding' );
				...
				$home_link = $this->wedding_get_single_breadcrumb_markup( $blog_title );
		...
		private function wedding_get_post_terms() {
			...
			if ( count( $terms ) == 1 ) {
				...
				if ( $term_parent ) {
					...
					foreach ( $term_tree as $term_id ) {
						...
						$terms_markup .= $this->wedding_get_single_breadcrumb_markup( $term_object->name, get_term_link( $term_object ) );
				...
				// We have a single term, so put it out
					...
					$terms_markup = $this->wedding_get_single_breadcrumb_markup( $terms[0]->name, get_term_link( $terms[0] ) );
			...
			// The post has multiple terms
				...
				$terms_markup = $this->wedding_get_single_breadcrumb_markup( $terms[0]->name, get_term_link( $terms[0] ), FALSE );
				...
				foreach ( $terms as $term ) {
					...
					if ( ++$i == $max_index ) {
						...
						$terms_markup .= ', '  . $this->wedding_get_single_breadcrumb_markup( $term->name, get_term_link( $term ), TRUE, FALSE );
						...
						$terms_markup .= ', '  . $this->wedding_get_single_breadcrumb_markup( $term->name, get_term_link( $term ), FALSE, FALSE );
		...
		private function wedding_get_post_ancestors() {
			...
			foreach ( $post_ancestor_ids as $post_ancestor_id ) {
				...
				$ancestors_markup .= $this->wedding_get_single_breadcrumb_markup( $post_ancestor->post_title, get_permalink( $post_ancestor->ID ) );
		...
		private function wedding_get_taxonomies() {
			...
			if ( $term->parent != 0 &&
				...
				foreach ( $term_ancestors as $term_ancestor ) {
					...
					$terms_markup .= $this->wedding_get_single_breadcrumb_markup( $term_object->name, get_term_link( $term_object->term_id, $term->taxonomy ) );
		...
		private function wedding_get_post_type_archive( $linked = TRUE ) {
			...
			if ( is_object( $post_type_object ) ) {
				...
				if ( class_exists( 'bbPress' ) && $post_type == 'topic' ) {
					...
					return $this->wedding_get_single_breadcrumb_markup( $archive_title, $link );
		...
		private function wedding_get_bbpress_main_archive_page() {
			...
			return $this->wedding_get_single_breadcrumb_markup( bbp_get_forum_archive_title(), get_post_type_archive_link( 'forum' ) );
		...
		private function wedding_get_breadcrumb_leaf_markup( $object_type = '' ) {
		...
		private function wedding_get_single_breadcrumb_markup( $title, $link = '', $separator = TRUE, $microdata = TRUE ) {
