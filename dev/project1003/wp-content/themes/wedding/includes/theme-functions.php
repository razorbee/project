...
if ( function_exists('wp_nav_menu') ) {
	...
	register_nav_menus( array( 
		...
		'primary-right'	=> esc_html__( 'Primary Right Menu', 'wedding' ),
		'top-menu'		=> esc_html__( 'Top Bar Menu', 'wedding' ),
		'mobile-menu'	=> esc_html__( 'Mobile Menu', 'wedding' ),
 ...
 */ 
if ( ! function_exists( 'wedding_content_sidebar_classes' ) ) { 

	function wedding_content_sidebar_classes() {	
 ...
 */
if ( ! function_exists( 'wedding_primary_content_classes' ) ) { 

	function wedding_primary_content_classes() {	
 ...
 */
if ( ! function_exists( 'wedding_primary_sidebar_classes' ) ) { 

	function wedding_primary_sidebar_classes() {	
 ...
 */
if ( ! function_exists( 'wedding_secondary_sidebar_classes' ) ) { 

	function wedding_secondary_sidebar_classes() {	
 ...
 */
if ( ! function_exists( 'wedding_footer_widget_column_classes' ) ) { 

	function wedding_footer_widget_column_classes( $column = 'no', $class = 'yes' ) {
		...
		if( isset( $footer_type ) && $footer_type == '' || $footer_type == 'default' ) {
			$footer_type = wedding_get_theme_option( 'zozo_footer_type' );
		}
...
if ( ! function_exists( 'wedding_display_social_icons' ) ) { 

	function wedding_display_social_icons( $type = '' ) {
		
		$output = '';
		$social_links = array();
		
		if( $type == '' ) {
			$type = wedding_get_theme_option( 'zozo_social_icon_type' );
		}
		
		if( $facebook_link = wedding_get_theme_option( 'zozo_facebook_link' ) ) {
			$social_links['facebook'] = $facebook_link;
		}
		
		if( $twitter_link = wedding_get_theme_option( 'zozo_twitter_link' ) ) {
			$social_links['twitter'] = $twitter_link;
		}
		
		if( $linkedin_link = wedding_get_theme_option( 'zozo_linkedin_link' ) ) {
			$social_links['linkedin'] = $linkedin_link;
		}
		
		if( $pinterest_link = wedding_get_theme_option( 'zozo_pinterest_link' ) ) {
			$social_links['pinterest'] = $pinterest_link;
		}
		
		if( $googleplus_link = wedding_get_theme_option( 'zozo_googleplus_link' ) ) {
			$social_links['google-plus'] = $googleplus_link;
		}
		
		if( $youtube_link = wedding_get_theme_option( 'zozo_youtube_link' ) ) {
			$social_links['youtube'] = $youtube_link;
		}
		
		if( $rss_link = wedding_get_theme_option( 'zozo_rss_link' ) ) {
			$social_links['rss'] = $rss_link;
		}
		
		if( $tumblr_link = wedding_get_theme_option( 'zozo_tumblr_link' ) ) {
			$social_links['tumblr'] = $tumblr_link;
		}
		
		if( $reddit_link = wedding_get_theme_option( 'zozo_reddit_link' ) ) {
			$social_links['reddit'] = $reddit_link;
		}
		
		if( $dribbble_link = wedding_get_theme_option( 'zozo_dribbble_link' ) ) {
			$social_links['dribbble'] = $dribbble_link;
		}
		
		if( $digg_link = wedding_get_theme_option( 'zozo_digg_link' ) ) {
			$social_links['digg'] = $digg_link;
		}
		
		if( $flickr_link = wedding_get_theme_option( 'zozo_flickr_link' ) ) {
			$social_links['flickr'] = $flickr_link;
		}
		
		if( $instagram_link = wedding_get_theme_option( 'zozo_instagram_link' ) ) {
			$social_links['instagram'] = $instagram_link;
		}
		
		if( $vimeo_link = wedding_get_theme_option( 'zozo_vimeo_link' ) ) {
			$social_links['vimeo'] = $vimeo_link;
		}
		
		if( $skype_link = wedding_get_theme_option( 'zozo_skype_link' ) ) {
			$social_links['skype'] = $skype_link;
		}
		
		if( $blogger_link = wedding_get_theme_option( 'zozo_blogger_link' ) ) {
			$social_links['blogger'] = $blogger_link;
		}
		
		if( $yahoo_link = wedding_get_theme_option( 'zozo_yahoo_link' ) ) {
...
function wedding_get_gallery_post_images($size, $id, $echo = 'yes') {
...
if ( ! function_exists( 'wedding_display_social_sharing_icons' ) ) { 

	function wedding_display_social_sharing_icons( $echo = 'yes' ) {
	
		global $post;
		
		$output = '';
				
		if( wedding_get_theme_option( 'zozo_sharing_facebook' ) ) {
		...
		if( wedding_get_theme_option( 'zozo_sharing_twitter' ) ) {
		...
		if( wedding_get_theme_option( 'zozo_sharing_linkedin' ) ) {
		...
		if( wedding_get_theme_option( 'zozo_sharing_pinterest' ) ) {
		...
		if( wedding_get_theme_option( 'zozo_sharing_googleplus' ) ) {
		...
		if( wedding_get_theme_option( 'zozo_sharing_tumblr' ) ) {
		...
		if( wedding_get_theme_option( 'zozo_sharing_reddit' ) ) {
		...
		if( wedding_get_theme_option( 'zozo_sharing_digg' ) ) {
		...
		if( wedding_get_theme_option( 'zozo_sharing_email' ) ) {
...
if ( ! function_exists( 'wedding_pagination' ) ) { 
	...
	function wedding_pagination( $pages = '', $scroll = '', $paged = '' ) {
...
if( ! function_exists( 'wedding_postnavigation' ) ) {
	function wedding_postnavigation() { 
		if ( is_single() ) { 
		?>
...
if( ! function_exists( "wedding_custom_comments" ) ) {

	function wedding_custom_comments( $comment, $args, $depth ) {
	      	...
	      	<div id="li-comment-<?php comment_ID() ?>" class="comment-container media">
				...
				<div class="comment-status-text media-body" id="comment-<?php comment_ID(); ?>">
					...
					<?php if ($comment->comment_approved == '0') { ?>
						<p class='comment-unapproved'><?php esc_html_e('Your comment is awaiting moderation.', 'wedding'); ?></p>
					<?php } ?>
					<div class="comments-box">
						<div class="comment-list meta">
							<span class="author-name"><i class="fa fa-user"></i> <?php echo get_comment_author_link() ?></span>
							<span class="date"><i class="fa fa-calendar"></i><?php printf(__('%1$s', 'wedding'), get_comment_date() ) ?></span>								
						</div>
					</div><!-- .comments-box -->
					<div class="reply">
						<span class="edit"><?php edit_comment_link( esc_html__('Edit', 'wedding'), '<i class="fa fa-edit"></i>', ''); ?></span>	
						<span class="reply-link"><?php comment_reply_link(array_merge( $args, array('reply_text' => esc_html__('Reply', 'wedding'), 'depth' => $depth, 'max_depth' => $args['max_depth'], 'before' => '<i class="fa fa-comments"></i>'))) ?></span>
					</div>
...
if( ! function_exists( "wedding_author_info" ) ) {

	function wedding_author_info() { 
		if( is_author() ) {
			...
			<div class="author-info-page">
				...
				<div class="author-title">
					<h4><?php esc_html_e('About', 'wedding'); ?> <span class="author-name"><?php echo esc_html($author_name); ?></span></h4>
				</div>
			  	<div class="author-description">
					<p>
					<?php if( !$author_description ) {
						echo sprintf( esc_html__( 'This author %s has created %s entries.', 'wedding' ), $author_name, count_user_posts( $author_id ) );
					} else {
 ...
 * ========================================================= */
if ( ! function_exists( 'wedding_get_fontawesome_icon_array' ) ) {
	function wedding_get_fontawesome_icon_array() {
 ...
 * ========================================================= */
if ( ! function_exists( 'wedding_get_glyphicons_array' ) ) {
	function wedding_get_glyphicons_array() {
	
		$pattern = '/\.(glyphicon-(?:\w+(?:-)?)+):before\s+{\s*content:\s*"(.+)";\s+}/';
		$glyphicon_path = WEDDING_THEME_URL . '/css/wedding-bootstrap.css';
 ...
 * ========================================================= */
if ( ! function_exists( 'wedding_get_taxonomy_terms_array' ) ) {
	function wedding_get_taxonomy_terms_array($taxonomy, $post_type, $msg) {
 ...
 * ========================================================= */
if ( ! function_exists( 'wedding_set_post_views_count' ) ) {
	function wedding_set_post_views_count() {
		global $post;
...
add_action('wp_head', 'wedding_set_post_views_count');

/* ==================================================================
 * Add Current Category Class to Categories List on Single Post
 * ================================================================== */
if ( ! function_exists( 'wedding_current_cat_on_single_posts' ) ) {

	function wedding_current_cat_on_single_posts($output) {
		global $post;
...
add_filter('wp_list_categories', 'wedding_current_cat_on_single_posts');
...
function wedding_color_luminance( $hex, $percent ) {
 ...
 * ================================================================== */
if ( ! function_exists( 'wedding_get_parallax_link' ) ) {
	function wedding_get_parallax_link( $item ) {
		global $wp_query;		
 ...
 * ================================================================== */
if ( ! function_exists( 'wedding_parallax_front_query' ) ) {

	function wedding_parallax_front_query() {
 ...
 * ================================================================== */
if ( ! function_exists( 'wedding_parallax_additional_query' ) ) {

	function wedding_parallax_additional_query( $ids ) {
...
function wedding_call_api( $method, array $data = array() ) {

	if( wedding_get_theme_option( 'zozo_mailchimp_api' ) != '' ) {
		$data['apikey'] = wedding_get_theme_option( 'zozo_mailchimp_api' );
	}
...
function wedding_get_mailing_lists() {
	...
	$connection_result = wedding_call_api( 'helper/ping' );
	...
	$result = wedding_call_api( 'lists/list', $args );
...
function wedding_get_mailing_lists_format() {
	
	$new_key = $old_mc_api_key = '';

	if( wedding_get_theme_option( 'zozo_mailchimp_api' ) != '' ) {
		$new_key = wedding_get_theme_option( 'zozo_mailchimp_api' );
	}
	...
	if( false === $cached_mc_lists || empty( $cached_mc_lists ) ) {
		$get_lists = wedding_get_mailing_lists();
...
function wedding_get_current_url() {
	global $wp;
...
function wedding_mc_subscribeformat( $email, $merge_vars, $mc_list_id ) {		
			
	$subscribe_result = wedding_mc_subscribe( $mc_list_id, $email, $merge_vars, 'html', true );
	
	return $subscribe_result;
	
}

function wedding_mc_subscribe( $list_id, $email, array $merge_vars = array(), $email_type = 'html', $double_optin = true, $update_existing = false, $replace_interests = true, $send_welcome = false )	{
	...
	$result = wedding_call_api( 'lists/subscribe', $data );

	if( is_object( $result ) ) {

		if( ! isset( $result->error ) ) {		               
			$mailchimp_success_msg = wedding_get_theme_option( 'zozo_mailchimp_success_message_api' ); 
			return $mailchimp_success_msg;
		} else {

			// Check subscription Error
			if( (int) $result->code === 214 ) {				
				$mailchimp_subscription_error_msg = wedding_get_theme_option( 'zozo_mailchimp_subscription_error_message_api' ); 
				return $mailchimp_subscription_error_msg;
			} 
		
			// Get Error Message
			$error_message = $result->error;			
			$mailchimp_error_msg = wedding_get_theme_option( 'zozo_mailchimp_error_message_api' ); 			
...
add_action('wp_ajax_zozo_mailchimp_subscribe', 'wedding_mailchimp_subscribe_user');
add_action('wp_ajax_nopriv_zozo_mailchimp_subscribe', 'wedding_mailchimp_subscribe_user');

if( ! function_exists( "wedding_mailchimp_subscribe_user" ) ) {

	function wedding_mailchimp_subscribe_user() {
		...
		$list_result = wedding_mc_subscribeformat( $email, $merge_vars, $mailing_list );
...
if( ! function_exists( 'wedding_related_portfolio_slider' ) ) {
	function wedding_related_portfolio_slider() {
	
		global $post;
		
		$items = $items_scroll = $itemstablet = $itemsmobileland = $itemsmobile = $pagination = $navigation = $margin = $autoplay = $duration = $loop = '';
		
		if( wedding_get_theme_option( 'zozo_portfolio_citems' ) != '' ) {
			$items = wedding_get_theme_option( 'zozo_portfolio_citems' );
		} else {
			$items = 3;
		}
		
		if( wedding_get_theme_option( 'zozo_portfolio_citems_scroll' ) != '' ) {
			$items_scroll = wedding_get_theme_option( 'zozo_portfolio_citems_scroll' );
		} else {
			$items_scroll = 1;
		}
		
		if( wedding_get_theme_option( 'zozo_portfolio_ctablet' ) != '' ) {
			$itemstablet = wedding_get_theme_option( 'zozo_portfolio_ctablet' );
		} else {
			$itemstablet = 2;
		}
		
		if( wedding_get_theme_option( 'zozo_portfolio_cmobile_land' ) != '' ) {
			$itemsmobileland = wedding_get_theme_option( 'zozo_portfolio_cmobile_land' );
		} else {
			$itemsmobileland = 2;
		}
		
		if( wedding_get_theme_option( 'zozo_portfolio_cmobile' ) != '' ) {
			$itemsmobile = wedding_get_theme_option( 'zozo_portfolio_cmobile' );
		} else {
			$itemsmobile = 1;
		}
		
		if( wedding_get_theme_option( 'zozo_portfolio_cpagination' ) == 1 ) {
			$pagination = "true";
		} else {
			$pagination = "false";
		}
		
		if( wedding_get_theme_option( 'zozo_portfolio_cnavigation' ) == 1 ) {
			$navigation = "true";
		} else {
			$navigation = "false";
		}
		
		if( wedding_get_theme_option( 'zozo_portfolio_cmargin' ) != '' ) {
			$margin = wedding_get_theme_option( 'zozo_portfolio_cmargin' );
		} else {
			$margin = 20;
		}
		
		if( wedding_get_theme_option( 'zozo_portfolio_cautoplay' ) == 1 ) {
			$autoplay = "true";
		} else {
			$autoplay = "false";
		}
		
		if( wedding_get_theme_option( 'zozo_portfolio_ctimeout' ) != '' ) {
			$duration = wedding_get_theme_option( 'zozo_portfolio_ctimeout' );
		} else {
			$duration = 5000;
		}
		
		if( wedding_get_theme_option( 'zozo_portfolio_cloop' ) == 1 ) {
			$loop = "true";
		} else {
			$loop = "false";
		}
		...
		if( isset( $items ) && $items == "1" ) {
			...
			$image_size = 'wedding-portfolio-mid';
		}
			...
			if( $portfolio_slider_query->have_posts() ) { ?>
				<div class="related-portfolio-slider-section">	
					...
					<div class="zozo-parallax-header">
						...
						<?php if( wedding_get_theme_option( 'zozo_portfolio_related_title' ) != '' ) { ?>
							<h2 class="parallax-title"><?php echo esc_html( wedding_get_theme_option( 'zozo_portfolio_related_title' ) ); ?></h2>
						<?php } else { ?>
							<h2 class="parallax-title"><?php esc_html_e( 'Related Projects', 'wedding'); ?></h2>
						<?php } ?>
						</div>
					</div>
								...
								<div class="portfolio-overlay">
									...
									$portfolio_large = wp_get_attachment_image_src(get_post_thumbnail_id($post->ID), 'full'); ?>
										...
										<div class="portfolio-title">
											<h4><?php the_title(); ?></h4>
											<p><?php echo wedding_shortcode_stripped_excerpt( get_the_content(), 8 ); ?></p>
										</div>
 ...
 * ============================================================= */
if( ! function_exists('wedding_woo_build_query_string') ) {
	function wedding_woo_build_query_string($params = array(), $overwrite_key, $overwrite_value) {
 ...
 * ============================================================= */
if( ! function_exists('wedding_expanded_allowed_tags') ) {
	function wedding_expanded_allowed_tags() {
...
if( ! function_exists( 'wedding_shortcodes_formatter' ) ) {
	function wedding_shortcodes_formatter( $content ) {
...
add_filter('the_content', 'wedding_shortcodes_formatter');
add_filter('widget_text', 'wedding_shortcodes_formatter');

/* =============================================================
 *	Render Breadcrumbs
 * ============================================================= */
 
if( ! function_exists( 'wedding_breadcrumbs' ) ) {
	function wedding_breadcrumbs() {
		wedding_zozo_breadcrumbs();
	}
}
...
if( ! is_admin() ) {
	function Wedding_SearchFilter($query) {
		...
		if( is_search() && $query->is_search ) {
			...
			if( wedding_get_theme_option( 'zozo_search_results_content' ) == 'only_posts') {
				$query->set('post_type', 'post');
			}
	
			if( wedding_get_theme_option( 'zozo_search_results_content' ) == 'only_pages') {
...
if ( ! function_exists( 'wedding_save_post_content_check' ) ) {
	function wedding_save_post_content_check( $post_id ) {
	...
	add_action( 'save_post', 'wedding_save_post_content_check' );
}
...
if ( ! function_exists( 'wedding_portfolio_media_output' ) ) {
	function wedding_portfolio_media_output( $post_id, $style, $columns, $portfolio_id, $item_link = '', $overlay_animation = '', $overlay_style = '' ) {
		...
		if( isset( $style ) && $style == 'masonry' ) {
			...
			$image_size = 'wedding-custom-single';
		} else {
				...
				if( $zozo_portfolio['portfolio_image'][$opt] != '' || $zozo_portfolio['portfolio_video'][$opt] != '' ) {
					if( $zozo_portfolio['portfolio_image'][$opt] != '' ) {
						...
						if( isset( $style ) && $style == 'masonry' ) {
							...
							$portfolio_sized_image = wedding_aq_resize( $thumb_img_url, $thumb_width, $thumb_height, false, false );
...
if ( ! function_exists( 'wedding_portfolio_media_icon_output' ) ) {
	function wedding_portfolio_media_icon_output( $post_id, $portfolio_id ) {
...
if ( ! function_exists( 'wedding_get_portfolio_post_media' ) ) {
	function wedding_get_portfolio_post_media( $thumb_width = '', $thumb_height = '', $image_id, $thumb_size = '', $thumb_size_w = '', $thumb_size_h = '' ) {
	
		$thumb_img_url = wp_get_attachment_url( $image_id, 'full' );
		
		$image = array();
		if( function_exists('wedding_aq_resize') && ( $thumb_width != '' || $thumb_height != '' ) ) {
			$image        = wedding_aq_resize( $thumb_img_url, $thumb_width, $thumb_height, false, false );
		} else {
 ...
 * ============================================================= */
function wedding_is_events_archive() {
...
function wedding_output_blog_post_slider( $post_id, $image_size, $gallery_images_type = '', $layout = '' ) {
	...
	if( wedding_get_theme_option( 'zozo_blog_slideshow_autoplay' ) == 1 ) {
		$data_attr .= ' data-autoplay=true ';
	} else {
		$data_attr .= ' data-autoplay=false ';
	}
	
	if( wedding_get_theme_option( 'zozo_blog_slideshow_timeout' ) != '' ) {
		$data_attr .= ' data-timeout=' . wedding_get_theme_option( 'zozo_blog_slideshow_timeout' ) . ' ';
	}
	...
	if( is_single() ) {
		if( isset( $gallery_images_type ) && $gallery_images_type == 'carousel' ) {
			...
			if( wedding_get_theme_option( 'zozo_single_blog_citems' ) != '' ) {
				$gallery_attr .= ' data-items=' . wedding_get_theme_option( 'zozo_single_blog_citems' ) . ' ';
			}
			
			if( wedding_get_theme_option( 'zozo_single_blog_citems_scroll' ) != '' ) {
				$gallery_attr .= ' data-slideby=' . wedding_get_theme_option( 'zozo_single_blog_citems_scroll' ) . ' ';
			}
			
			if( wedding_get_theme_option( 'zozo_single_blog_ctablet' ) != '' ) {
				$gallery_attr .= ' data-md=' . wedding_get_theme_option( 'zozo_single_blog_ctablet' ) . ' ';
			}
			
			if( wedding_get_theme_option( 'zozo_single_blog_cmlandscape' ) != '' ) {
				$gallery_attr .= ' data-sm=' . wedding_get_theme_option( 'zozo_single_blog_cmlandscape' ) . ' ';
			}
			
			if( wedding_get_theme_option( 'zozo_single_blog_cmportrait' ) != '' ) {
				$gallery_attr .= ' data-xs=' . wedding_get_theme_option( 'zozo_single_blog_cmportrait' ) . ' ';
			}
			
			if( wedding_get_theme_option( 'zozo_single_blog_cautoplay' ) == 1 ) {
				$gallery_attr .= ' data-autoplay=true ';
			} else {
				$gallery_attr .= ' data-autoplay=false ';
			}
			
			if( wedding_get_theme_option( 'zozo_single_blog_ctimeout' ) != '' ) {
				$gallery_attr .= ' data-timeout=' . wedding_get_theme_option( 'zozo_single_blog_ctimeout' ) . ' ';
			}
			
			if( wedding_get_theme_option( 'zozo_single_blog_cloop' ) == 1 ) {
				$gallery_attr .= ' data-loop=true ';
			} else {
				$gallery_attr .= ' data-loop=false ';
			}
			
			if( wedding_get_theme_option( 'zozo_single_blog_cmargin' ) != '' ) {
				$gallery_attr .= ' data-margin=' . wedding_get_theme_option( 'zozo_single_blog_cmargin' ) . ' ';
			}
			
			if( wedding_get_theme_option( 'zozo_single_blog_cdots' ) == 1 ) {
				$gallery_attr .= ' data-dots=true ';
			} else {
				$gallery_attr .= ' data-dots=false ';
			}
			
			if( wedding_get_theme_option( 'zozo_single_blog_cnav' ) == 1 ) {
				...
				$output .= '<div class="entry-thumbnail-wrapper nested-carousel zozo-owl-wrapper">';
					...
					if( is_single() && isset( $gallery_images_type ) && $gallery_images_type == 'carousel' ) {
						$output .= '<div id="zozo_owl_' . $post_id .'" class="entry-thumbnail owl-carousel blog-single-carousel-slider"'. esc_attr( $gallery_attr ) .'>';
							$output .= wedding_get_gallery_post_images( 'wedding-blog-medium', $post_id, 'no' );
						$output .= '</div>';
					} else {
						$output .= '<div id="zozo_owl_' . $post_id .'" class="entry-thumbnail owl-carousel blog-carousel-slider"'. esc_attr( $data_attr ) .'>';
							$output .= wedding_get_gallery_post_images( $image_size, $post_id, 'no' );
						$output .= '</div>';
					}
					$output .= '</div>';
				$output .= '</div>';
...
function wedding_output_blog_large_layout( $post_id, $post_class, $image_size, $excerpt_limit, $meta_array, $layout ) {
	...
	$output .= '<div class="post-inner-wrapper">';
		...
		if( ! $meta_array['thumbnail'] ) {
			if( $layout == 'list' ) {
				$output .= wedding_output_blog_post_slider( $post_id, $image_size, '', 'grid' );
			} else {
				$output .= wedding_output_blog_post_slider( $post_id, $image_size );
			}
		}
		...
		$output .= '<div class="posts-content-container">';
			...
			if( has_post_format('quote') ) {
				...
				$output .= '<blockquote>';
					$output .= '<p>'. wedding_blog_trim_excerpt( $excerpt_limit ) .'</p>';
				...
				$output .= wp_link_pages( array(
							'before' 		=> '<div class="page-links"><span class="page-links-title">' . esc_html__( 'Pages:', 'wedding' ) . '</span>',
							'after' 		=> '</div>',
							'link_before' 	=> '<span>',
							'link_after' 	=> '</span>',
							'echo' 			=> 0
			...
			$output .= '<div class="entry-meta-wrapper">';
				$output .= '<div class="entry-meta">';
					...
					if ( is_sticky() && is_home() && ! is_paged() ) {
						$output .= '<div class="sticky-post"><i class="simple-icon icon-pin"></i></div>';
					}
				...
				if( ! $meta_array['more'] ) {
					$output .= '<div class="read-more">';
						...
						if( ! wedding_get_theme_option( 'zozo_blog_read_more_text' ) ) { 
							$output .= esc_html__('Read more', 'wedding'); 
						} else { 
							$output .= esc_attr( wedding_get_theme_option( 'zozo_blog_read_more_text' ) ); 
						}
...
function wedding_output_blog_grid_layout( $post_id, $post_class, $image_size, $excerpt_limit, $meta_array, $animation = array() ) {
...
/*Breadcrumbs*/
if( ! function_exists('wedding_zozo_breadcrumbs') ) {
	function wedding_zozo_breadcrumbs() {
	 
	  $showOnHome = 1; // 1 - show breadcrumbs on the homepage, 0 - don't show
	  $delimiter = ''; // delimiter between crumbs
	  $home = esc_html__('Home', 'wedding'); // text for the 'Home' link
	  ...
	  if (is_home() || is_front_page()) {
		...
		if ( is_category() ) {
		  ...
		  echo wp_kses_post( $before . esc_html__('Posts by ', 'wedding') . $userdata->display_name . $after );
	 
		} elseif ( is_404() ) {
		  echo wp_kses_post( $before . esc_html__('Error 404', 'wedding') . $after );
		}
	 
		if ( get_query_var('paged') ) {
		  if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo ' (';
		  echo esc_html__( 'Page', 'wedding' ) . ' ' . get_query_var('paged');
		  if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo ')';
		}
	  }
	  echo '</div>';
...
/*Save Theme Options*/
if( ! function_exists('wedding_save_theme_options') ) {
 function wedding_save_theme_options() {
  global $wp_filesystem;
  $upload_dir = wp_upload_dir();
  $cus_dir_name = $upload_dir['basedir'] . '/wedding';
