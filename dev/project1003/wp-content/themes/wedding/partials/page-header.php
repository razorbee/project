...
if( isset( $hide_title_bar ) && $hide_title_bar == '' || $hide_title_bar == 'default' ) {
	...
	$hide_title_bar = wedding_get_theme_option( 'zozo_enable_page_title_bar' );
...
if( isset( $hide_title ) && $hide_title == '' || $hide_title == 'default' ) {
	...
	$hide_title = wedding_get_theme_option( 'zozo_enable_page_title' );
...
if( isset( $page_title_align ) && $page_title_align == '' || $page_title_align == 'default' ) {
	...
	$page_title_align = wedding_get_theme_option( 'zozo_page_title_alignment_new' );
...
if( isset( $page_title_type ) && $page_title_type == '' || $page_title_type == 'default' ) {
	...
	$page_title_type = wedding_get_theme_option( 'zozo_page_title_bar_type_new' );	
...
if( isset( $page_title_skin ) && $page_title_skin == '' || $page_title_skin == 'default' ) {
	...
	$page_title_skin = wedding_get_theme_option( 'zozo_page_title_bar_skin_new' );	
...
if( !$page_video_bg  ) {
	...
	$page_video_bg = wedding_get_theme_option( 'zozo_page_title_video_bg' );	
...
if( $page_video_id == ''  ) {
	...
	$page_video_id = wedding_get_theme_option( 'zozo_page_title_video_bg_id' );	
...
if( $page_title_bg == ''  ) {
	...
	$page_title_array_bg = wedding_get_theme_option( 'zozo_page_title_background_image' );
...
if( isset( $page_bg_parallax ) && $page_bg_parallax == '' || $page_bg_parallax == 'default' ) {
	...
	$page_bg_parallax = wedding_get_theme_option( 'zozo_page_title_parallax_bg_image' );
...
if( isset( $page_title_bar_bg_position ) && $page_title_bar_bg_position == '' || $page_title_bar_bg_position == 'default' ) {
	...
	$page_title_bar_bg_position = wedding_get_theme_option( 'zozo_page_title_background_position' );	
...
if( isset( $breadcrumbs ) && $breadcrumbs == '' || $breadcrumbs == 'default' ) {
	...
	$breadcrumbs = wedding_get_theme_option( 'zozo_enable_breadcrumbs' );
...
if( is_home() ) {
	...
	$title = wedding_get_theme_option( 'zozo_blog_title' );



	$page_slogan = wedding_get_theme_option( 'zozo_blog_slogan' );
...
if( is_search() ) {
	...
	$title = esc_html__( 'Search results for:', 'wedding' ) . ' ' . get_search_query();
...
if( is_404() ) {
	...
	$title = esc_html__('Error 404 Page', 'wedding');
...
if( ( class_exists( 'TribeEvents' ) && tribe_is_event() && ! is_single() && ! is_home() ) ||
	...
	( class_exists( 'TribeEvents' ) && wedding_is_events_archive() ) ||



	( class_exists( 'TribeEvents' ) && wedding_is_events_archive() && is_404() ) ) { 
...
if( is_archive() && ! ( class_exists('bbPress') && is_bbpress() ) && ! is_search() ) {
	...
	if ( is_day() ) {
		...
		$title = esc_html__( 'Daily Archives:', 'wedding' ) . '<span> ' . get_the_date() . '</span>';
		...
		$title = esc_html__( 'Monthly Archives:', 'wedding' ) . '<span> ' . get_the_date( _x( 'F Y', 'monthly archives date format', 'wedding' ) ) . '</span>';
		...
		$title = esc_html__( 'Yearly Archives:', 'wedding' ) . '<span> ' . get_the_date( _x( 'Y', 'yearly archives date format', 'wedding' ) ) . '</span>';
...
if( WEDDING_WOOCOMMERCE_ACTIVE && is_woocommerce() && ( is_product() || is_shop() ) && ! is_search() ) {
...
if( ! is_archive() && ! is_search() && ! ( is_home() && ! is_front_page() ) ) {
	...
	if ( $hide_title_bar != 'yes' ) {
		...
		if( is_home() && is_front_page() && ! wedding_get_theme_option( 'zozo_blog_page_title_bar' ) ) {
	...
	if( is_home() && ! wedding_get_theme_option( 'zozo_blog_page_title_bar' ) ) {
...
if( isset( $page_bg_parallax ) && $page_bg_parallax == 'yes' ) {
	...
	wp_enqueue_script( 'wedding-zozo-skrollr-js' );
