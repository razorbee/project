...
class PrescriptionController extends Controller
{
    ...
    public function newPrescription()
    {
        ...
        $template = PrescriptionTemplate::select('id','name')->where('status',1)->orderBy('name','asc')->get();
        $patients = Patient::select('id','name','phone')->where('status',1)->orderBy('name','asc')->get();
